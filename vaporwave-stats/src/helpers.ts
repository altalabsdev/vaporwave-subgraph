...
export let VWAVE = '0x62edc0692bd897d2295872a9ffcac5425011c661';
...
export function getTokenPrice(token: String): BigInt {
  ...
  if (token != VWAVE) {
    ...
    if (chainlinkPriceEntity != null) {
      ...
      // adjusting them to fit VWAVE 30 decimals USD values
      return chainlinkPriceEntity.value * BigInt.fromI32(10).pow(22);
    }
  }

  if (token == VWAVE) {
    let uniswapPriceEntity = UniswapPrice.load(VWAVE);
  ...
  prices.set(VWAVE, BigInt.fromI32(30) * PRECISION);
