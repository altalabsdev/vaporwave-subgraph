...
export function handleUniswapVwaveEthSwap(event: UniswapSwap): void {
  let ethPerVwave =
    (-(
      ...
      BigInt.fromI32(100)) /
    BigInt.fromI32(99);
  let vwavePrice = getTokenAmountUsd(WETH, ethPerVwave);

  let totalId = VWAVE;
  _storeUniswapPrice(totalId, VWAVE, vwavePrice, 'last', event.block.timestamp);

  let id = VWAVE + ':' + event.block.timestamp.toString();
  _storeUniswapPrice(id, VWAVE, vwavePrice, 'any', event.block.timestamp);
}
