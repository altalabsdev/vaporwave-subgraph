...
import {
  ...
  AddLiquidity,
  RemoveLiquidity,
} from '../generated/VlpManager/VlpManager';

import { Distribute } from '../generated/FeeVwaveRewardDistributor/RewardDistributor';
...
export function handleBuy(event: BuyEvent): void {
  let volume = event.params.usdAmount * BigInt.fromString('1000000000000');
...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeVlpStat(
    event.block.timestamp,
    event.params.vlpSupply,
    event.params.aumInUsd
  );
}
...
export function handleDistributeEthToVwave(event: Distribute): void {
  let amount = event.params.amount;
  let amountUsd = getTokenAmountUsd(WETH, amount);
  let totalEntity = _getOrCreateVwaveStat('total', 'total');
  ...
  let entity = _getOrCreateVwaveStat(id, 'daily');
...
export function handleDistributeEsvwaveToVwave(event: Distribute): void {
  let amount = event.params.amount;
  let amountUsd = getTokenAmountUsd(VWAVE, amount);

  let totalEntity = _getOrCreateVwaveStat('total', 'total');
  totalEntity.distributedEsvwave += amount;
  totalEntity.distributedEsvwaveCumulative += amount;
  totalEntity.distributedEsvwaveUsd += amountUsd;
  totalEntity.distributedEsvwaveUsdCumulative += amountUsd;
  ...
  entity.distributedEsvwave += amount;
  entity.distributedEsvwaveCumulative = totalEntity.distributedEthCumulative;
  entity.distributedEsvwaveUsd += amountUsd;
  entity.distributedEsvwaveUsdCumulative = totalEntity.distributedUsdCumulative;
...
function _getOrCreateTokenStat(
  ...
  if (entity == null) {
    ...
    entity.usdAmount = BigInt.fromI32(0);
  }
  return entity as TokenStat;
}

function _getOrCreateVwaveStat(id: string, period: string): VwaveStat {
  let entity = VwaveStat.load(id);
  if (entity == null) {
    entity = new VwaveStat(id);
    ...
    entity.distributedEsvwave = ZERO;
    entity.distributedEsvwaveCumulative = ZERO;
    entity.distributedEsvwaveUsd = ZERO;
    entity.distributedEsvwaveUsdCumulative = ZERO;
    entity.period = period;
  }
  return entity as VwaveStat;
}
...
function _getOrCreateVlpStat(
  id: string,
  period: string,
  periodTimestmap: i32
): VlpStat {
  let entity = VlpStat.load(id);
  if (entity == null) {
    entity = new VlpStat(id);
    entity.period = period;
    entity.vlpSupply = ZERO;
    entity.aumInUsd = ZERO;
    entity.distributedEth = ZERO;
    entity.distributedEthCumulative = ZERO;
    entity.distributedUsd = ZERO;
    entity.distributedUsdCumulative = ZERO;
  ...
  return entity as VlpStat;
}

export function handleDistributeEthToVlp(event: Distribute): void {
  let amount = event.params.amount;
  let amountUsd = getTokenAmountUsd(WETH, amount);
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32;

  let totalEntity = _getOrCreateVlpStat('total', 'total', periodTimestamp);
  totalEntity.distributedEth += amount;
  totalEntity.distributedEthCumulative += amount;
  totalEntity.distributedUsd += amountUsd;
  totalEntity.distributedUsdCumulative += amountUsd;
  ...
  let id = periodTimestamp.toString() + ':daily';
  let entity = _getOrCreateVlpStat(id, 'daily', periodTimestamp);
...
export function handleDistributeEsvwaveToVlp(event: Distribute): void {
  let amount = event.params.amount;
  let amountUsd = getTokenAmountUsd(VWAVE, amount);
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32;
...
function _storeVlpStat(
  ...
  let periodTimestamp = parseInt(_getDayId(timestamp)) as i32;
  let totalEntity = _getOrCreateVlpStat('total', 'total', periodTimestamp);
  totalEntity.aumInUsd = aumInUsd;
  totalEntity.vlpSupply = vlpSupply;
  totalEntity.save();

  let id = periodTimestamp.toString() + ':daily';
  let entity = _getOrCreateVlpStat(id, 'daily', periodTimestamp);
  entity.aumInUsd = aumInUsd;
  entity.vlpSupply = vlpSupply;
  entity.save();
}
